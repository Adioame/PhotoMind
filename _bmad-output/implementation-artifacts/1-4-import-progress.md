...
- [x] Phase 1: 创建 importProgressService.ts
- [x] Phase 2: 更新 importService.ts 集成进度服务
- [x] Phase 3: 更新 main/index.ts IPC 处理器
- [x] Phase 4: 创建 importStore.ts 前端状态管理
- [x] Phase 5: 创建 ImportProgress.vue 组件
- [x] Phase 6: 运行 TypeScript 编译检查

## Dev Agent Record

### Implementation Notes

1. **importProgressService.ts**:
   - 基于 EventEmitter 实现，支持多监听器
   - 支持 6 个阶段: scanning → preparing → importing → metadata → thumbnails → complete
   - 自动计算预计剩余时间
   - 错误追踪（最多 20 条）
   - 进度更新频率限制（每秒最多 2 次）

2. **importService.ts 集成**:
   - 在扫描、准备、导入各阶段调用进度服务
   - 替代原有的简单回调机制

3. **前端 store (importStore.ts)**:
   - Pinia 状态管理
   - 计算属性: progressPercentage, isComplete, formattedTimeRemaining
   - 阶段标签本地化

4. **ImportProgress.vue 组件**:
   - 阶段指示器（带动画）
   - 当前文件名滚动显示
   - 统计卡片（成功/跳过/失败）
   - 时间估计
...
| 文件 | 操作 |
|------|------|
| `electron/services/importProgressService.ts` | 新建 |
| `electron/services/importService.ts` | 修改 |
| `electron/main/index.ts` | 修改 |
| `src/renderer/stores/importStore.ts` | 新建 |
| `src/renderer/components/import/ImportProgress.vue` | 新建 |
