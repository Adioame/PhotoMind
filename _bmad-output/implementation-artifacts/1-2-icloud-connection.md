...
- [x] Phase 1: 增强 iCloudService.ts - AppleScript 支持
- [x] Phase 2: 添加相册获取功能
- [x] Phase 3: 添加下载状态检测
- [x] Phase 4: 添加存储空间信息
...
1. **iCloudService.ts 增强**:
   - 添加 `checkAvailability()` - 检测 iCloud Photos 可用性
   - 添加 `getAlbums()` - 获取相册列表
   - 添加 `getPhotosWithStatus()` - 获取照片列表（带下载状态）
   - 添加 `downloadPhoto()` - 下载照片
   - 添加 `downloadPhotos()` - 批量下载
   - 添加 `getStorageInfo()` - 获取存储空间信息
   - 添加 `parseAlbumList()` - 解析 AppleScript 返回的相册
   - 添加 `parsePhotoList()` - 解析 AppleScript 返回的照片
   - 添加 `parseCloudStatus()` - 解析下载状态

2. **AppleScript 集成**:
   - 通过 `osascript` 执行 AppleScript 命令
   - 获取相册列表和照片信息
   - 触发照片下载

3. **备用方案**:
   - 如果 AppleScript 不可用，使用 `apple-photos-js` 库
   - 开发阶段使用模拟数据

### Technical Decisions

1. **多策略访问**: AppleScript → apple-photos-js → 模拟数据
2. **状态解析**: 解析 AppleScript 返回的文本格式
3. **存储信息**: 使用 `df -h` 命令获取磁盘空间

### File List

| 文件 | 操作 |
|------|------|
| `electron/services/iCloudService.ts` | 修改 |

### Completion Notes

Story E-01.2 实现完成。所有功能性需求和非功能性需求均已满足。

已实现功能:
- [x] 检测 macOS 系统和 iCloud Photos 可用性
- [x] 通过 AppleScript 访问 iCloud 照片库
- [x] 获取 iCloud 照片列表
- [x] 支持按相册筛选
- [x] 支持下载 iCloud 照片
- [x] 显示 iCloud 下载状态（已下载/在云端）
- [x] 支持批量下载
- [x] 显示存储空间信息
