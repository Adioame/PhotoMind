...
**原始需求描述**:
作为用户，我希望系统能够自动识别同一人物在不同照片中的人脸，并将它们归类到一起。

**描述**:
基于检测到的人脸特征，自动匹配相似人脸，实现人物照片的自动归类。
...
export class FaceMatchingService {
  ...
  private descriptorCache: Map<number, FaceDescriptor> = new Map()
  ...
  async findSimilarFaces(
    ...
    const { threshold = 0.6, minSimilarity = 0.5 } = options
